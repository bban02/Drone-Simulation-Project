...
void SimulationModel::scheduleTrip(JsonObject& details) {
  ...
  // changed to a robot decorator
  RobotDecorator* receiver = nullptr;

  for (auto& [id, entity] : entities) {
    if (name == entity->getName()) {
      if (Robot* r = dynamic_cast<Robot*>(entity)) {
        if (r->requestedDelivery) {
          // updated to wrap robot
          receiver = new RobotDecorator(r);
          break;
        }
      }
    }
  ...
  if (receiver && package) {
    ...
    DataSingleton* singleton = DataSingleton::getInstance(receiver);
  }
}
